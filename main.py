...
def main():
    while user_input:=input(constants.OPTIONS) != "5":
        ...
        if int(user_input) == 1:
            ...
            for page in tqdm(links_pages):
                ...
                get_books = GetBooks.GetBooksFromPage()
            ...
            for book in tqdm(get_books.books):
                ...
                soup = BeautifulSoup(req.text, 'html.parser')
                brain_of_books = LinkBrain.LinkBrain(book)
            ...
            for i,row in tqdm(data_books.iterrows()):
                url = row["image_url"]
                response = requests.get(url)
                title = row["title"].replace(" ", "_").replace(":", "_").replace("/", "_").replace(".", "_").replace(",", "_").replace("?", "_")
